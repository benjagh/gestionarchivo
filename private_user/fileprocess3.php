...
if (isset($_POST['save'])) { // if the save button on the form is clicked
    ...
    if (!in_array($extension, $allowed_extensions)) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM `upload_info_temuco` WHERE `name` = '$filename'") or die(mysqli_error($conn));
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO upload_info_temuco (name, size, download, timers, admin_status, email, sede, ramo_id) VALUES ('$filename', $size, 0, '$time', 'Ingeniería Civil en Informática', '$userEmail', 'Temuco', '$ramoId')";
