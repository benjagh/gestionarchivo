...
if (isset($_POST['ramoId']) && isset($_POST['email'])) {
    ...
    $query = "SELECT * FROM upload_info_talca WHERE ramo_id = ? AND EMAIL = ? ORDER BY NAME DESC";
    if ($stmt = mysqli_prepare($conn, $query)) {
        ...
        while ($file = mysqli_fetch_assoc($result)) {
            ...
            echo "<td class='text-center'><a href='downloads.php?file_id=" . htmlspecialchars($file['id']) . "'><img src='img/download-icon.png' width='30px' height='30px' title='Download File'></a></td>";
            echo "</td>";
